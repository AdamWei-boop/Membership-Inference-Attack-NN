...
def get_cmd_arguments():
    ...
    parser.add_argument('--t_train_dir', default='.', type=str)
    parser.add_argument('--t_out_dir', default='.',type=str)
    parser.add_argument('--s_train_dir', default='.', type=str)
    parser.add_argument('--s_out_dir', default='.',type=str)   
    parser.add_argument('--is_DP', default=False)
    parser.add_argument('--eps', default=2, help='Differential privacy level')        
    parser.add_argument('--dataset', default='MNIST', type=str, choices=['CIFAR10', 'MNIST'], help='Which dataset to use (CIFAR10 or MNIST)')
    parser.add_argument('--dataPath', default='D:/WORK/Scholar/Datasets/', type=str, help='Path to store data')
    parser.add_argument('--modelPath', default='./model',type=str, help='Path to save or load model checkpoints')
    parser.add_argument('--trainTargetModel', default=False, help='Train a target model, if false then load an already trained model')
    parser.add_argument('--trainShadowModel', default=False, help='Train a shadow model, if false then load an already trained model')
    parser.add_argument('--need_augm', default=False, help='To use data augmentation on target and shadow training set or not')
    parser.add_argument('--need_topk', default=False, help='Flag to enable using Top 3 posteriors for attack data')
    parser.add_argument('--param_init', default=False, help='Flag to enable custom model params initialization')
    parser.add_argument('--verbose', default=False, help='Add Verbosity')    
...
def split_dataset(total_size):
    ...
    split1 = int(0.8*total_size/10)
    split2 = int(total_size/10)
    split3 = int(0.8*(total_size-split2)) + split2
    ...
    t_test_idx = indices[split1:split2]

    #Target model train and test set
    s_train_idx = indices[split2:split3]
    s_test_idx = indices[split3:]
...
def get_data_loader(dataset,
    ...
    s_train_idx, s_out_idx, t_train_idx, t_out_idx = \
                    set_indices[0], set_indices[1], set_indices[2], set_indices[3]
    ...
    return t_train_loader, t_val_loader, t_out_loader, s_train_loader,\
           s_val_loader, s_out_loader
...
def create_attack(dataset, dataPath, modelPath, trainTargetModel,\
                  trainShadowModel, need_augm, need_topk, param_init, verbose):
    ...
    if (trainTargetModel):
        ...
        s_train_idx, s_out_idx, t_train_idx, t_out_idx = split_dataset(total_size)
        
        
        set_indices.append(s_train_idx)
        set_indices.append(s_out_idx)
        set_indices.append(t_train_idx)
        set_indices.append(t_out_idx)
        
        # Creating data loaders
        t_train_loader, t_val_loader, t_test_loader,\
        s_train_loader, s_val_loader, s_test_loader\
        ...
        if dataset == 'CIFAR10':            
            ...
            target_model = model.CNNCifar(num_classes).to(device)
        else:
            # target_model = model.MNISTNet(input_dim, n_hidden_mnist, num_classes).to(device)
            target_model = model.CNNMnist(num_classes).to(device)
        ...
        if args.is_DP:
            ...
            sample_rate = batch_size/len(s_train_idx)
            max_grad_norm = 20.0
        ...
        t_train_idx = torch.load(args.t_train_dir)
        t_out_idx = torch.load(args.t_out_dir)
        
        s_train_idx = torch.load(args.s_train_dir)
        s_out_idx = torch.load(args.s_out_dir)
        
        set_indices = []
        
        set_indices.append(list(s_train_idx))
        set_indices.append(list(s_out_idx))
        set_indices.append(list(t_train_idx))
        set_indices.append(list(t_out_idx))
        ...
        if dataset == 'CIFAR10':
            ...
            target_model = model.MNISTNet(input_dim,n_hidden_mnist,num_classes).to(device)
            # target_model = model.MLP1(dim_in=784, dim_hidden=256,\
            #                                 dim_out=num_classes).to(device)
            # target_model = model.CNNMnist(num_classes).to(device)

        target_model.load_state_dict(torch.load(args.target_file))
    ...
    if (trainShadowModel):
        ...
        if dataset == 'CIFAR10':
            ...
            shadow_model = model.CNNMnist(num_classes).to(device)
        ...
        if dataset == 'CIFAR10':
            ...
            # shadow_model = model.MLP1(dim_in=784, dim_hidden=256,\
            #                                 dim_out=num_classes).to(device)
    ...
    attack_optimizer = torch.optim.Adam(attack_model.parameters(), 
                                        lr=LR_ATTACK, 
                                        weight_decay=REG)
    attack_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(attack_optimizer,
                                                                 gamma=LR_DECAY)
    ...
    attack_valacc = train_attack_model(attack_model, 
                                       attackdataset, 
                                       attack_loss,
                                       attack_optimizer, 
                                       attack_lr_scheduler, 
    ...
    attack_acc = attack_inference(attack_model, targetX, targetY, device)
...
if __name__ == '__main__':
    ...
    set_eps = [1,5,10,50]
    attack_acc_list = []
    for args.eps in set_eps:
        attack_acc = create_attack(args.dataset, args.dataPath, args.modelPath, True, True,\
                      False, False, False, False)
        attack_acc_list.append(attack_acc)
    print('Eps:', args.eps, 'Attack acc:', attack_acc_list)
